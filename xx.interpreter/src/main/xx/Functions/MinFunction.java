...
/**
 * Function that returns the minimum value of two numbers
...
public class MinFunction extends GenericFunction {
	...
	/**
	 * Returns the minimum value given a list of parameters
	 * @param parameters the list of parameters
	 * @return the min value of the (two) parameters
	 */
	@Override
	public GenericAbstractElement getValue(List<GenericAbstractElement> parameters) {
		...
		if(parameters.size() == 2){
			...
			if(parameter0 instanceof GenericNumber && parameter1 instanceof GenericNumber){
				...
				return new GenericNumber(Math.min(number1.getValue(), number2.getValue()));
			}
			else{
